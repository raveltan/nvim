# ts-textobjects
> Treesitter textobject queries, motions, swaps, and incremental selection.
...
## Scope
Supplies the `textobjects.scm` queries (function, class, parameter), motion mappings to jump between them, and a swap module for arg/element reordering. Also wires a hand-rolled incremental selection that expands/shrinks by syntax node. The select textobjects themselves (`af`/`if`/`ac`/`ic`/`aa`/`ia`) are owned by mini.ai via `gen_spec.treesitter` ([editor-mini-ai](editor-mini-ai.md)) — mapping them here too would shadow mini.ai's versions and lose counts, `an`/`al` next/last variants, and dot-repeat.
...
```lua
{
  ...
  config = function()
    ...
    -- per-key map() calls into move/swap modules (select maps live in mini.ai)
  end,
}
```
...
## Our config
- `select = { lookahead = true }` — kept in `setup()`, but no select maps are bound here; the equivalent forward-seek comes from mini.ai's `cover_or_next` search.
- Mappings called explicitly per key via `move.goto_next_start`/`goto_previous_start`, `swap.swap_next`/`swap.swap_previous` — no module-level `keymaps = {...}` block.
...
## Notes
- Select textobjects (`af`/`if`/`ac`/`ic`/`aa`/`ia`) moved to mini.ai (`gen_spec.treesitter`, lua/plugins/editor.lua) — gains counts, `an`/`in`/`al`/`il` variants, dot-repeat. This plugin still ships the queries they read.
