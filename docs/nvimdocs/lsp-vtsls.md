...
- Outside GAF, `flags.allow_incremental_sync = false` — full document sync. With `@vue/typescript-plugin` loaded, volar proxies the whole tsserver project service, and an incremental range that lands past tsserver's copy of a file kills the server: `Debug Failure. Bad line number. Line: N, lineStarts.length: N` → `Client vtsls quit with exit code 1`. Hit on plain `.ts` files too, not only SFCs. Full sync has no range arithmetic, so the copies cannot drift. GAF keeps incremental sync (large Angular files, no vue plugin).
