...
```lua
{
  ...
  opts = {
    ...
    keymaps = { focus_list = "<C-l>", focus_preview = "<C-p>" },
...
|---|---|---|---|
| `<leader><leader>` | n | `fff.find_files()` | Find files (primary) |
| `<leader>ff` | n | `fff.find_files()` | Find files (alias) |
...
| `<leader>sg` | n | `fff.live_grep()` | Live grep cwd |
