...
| `gd` | n | (GAF, api-repo python) cross-service thrift definition: call site → midlayer/dao handler; `conns.gaf.*` → fl-gaf PHP trait in vertical split; on own `def` → callers picker | gaf/python_nav.lua |
