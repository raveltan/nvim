...
-- rg over a search root, never an LSP: one hit jumps straight, several open a
-- Snacks picker.
local M = {}
...
-- Parse `rg --vimgrep` stdout into Snacks picker items.
...
function M.show(title, items, opts)
  ...
  Snacks.picker.pick(vim.tbl_deep_extend("force", {
    ...
    confirm = function(picker, item)
      picker:close()
      if item then M.jump_item(item) end
    end,
  }, opts.pick or {}))
end
