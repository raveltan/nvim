...
--
-- `gd` reads the injected `angular` tree, which is precise but needs a
-- well-formed template. Completion can't use it: a half-typed start tag is
-- unclosed, and treesitter's error recovery then resolves the cursor to the
-- nearest well-formed *enclosing* element (`<app-foo [` reads as the surrounding
-- `<div>`). So the completion reads scan backwards over the tag segment instead.
--
-- The segment is cached per (buffer, changedtick, cursor): one completion round
-- asks for the tag name, the quote state and the attribute being valued, and
-- rebuilding the 30-line window for each was the bulk of the per-keystroke work.
