...
-- A template expression can name something the template itself introduced: an
-- `@if (x$ | async; as foo)` alias, an `@for` loop variable, an `@let`, an
-- `*ngIf="e as v"` / `*ngFor="let i of ..."` binding, a `#ref`, or a
-- `<ng-template let-ctx>` var. These run before the class-member search, which
-- never finds them.
...
-- True when the cursor was on an Angular template target (tag, attribute, CSS
-- class, or a symbol in a binding expression) and this claimed the jump, even if
-- the target didn't resolve. False only on plain TypeScript, so the caller can
-- fall back to LSP `gd`.
function M.goto_definition()
  ...
  -- A symbol in a binding value/interpolation resolves to its TS definition,
  -- before tag/attribute classification. A name the template itself binds wins
  -- over the class-member search: that's its lexical declaration.
  ...
  if tgt.kind == "tag" then
    ...
    -- One file -> jump; several defining the same selector (projects and
    -- contests both define `app-collaborator-info`) -> picker.
...
-- Takes a class name (`FooComponent`, prefix ok) or a selector (`app-foo`),
