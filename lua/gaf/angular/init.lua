...
-- Feeds a blink.cmp source (lua/gaf/angular/inputs_source.lua): when the cursor is
...
-- ── inline-template tag completion ─────────────────────────────────────────
-- Typing `<` in an inline template offers every component in the repo. Unlike
-- the attribute completion above there is nothing under the cursor to rg for, so
-- the candidate set comes from the repo-wide selector index
-- (lua/gaf/angular/selector_index.lua), built on first use and patched on write.

-- Directory of `file` as an import-style path (`@freelancer/ui/button`), which
-- both labels an item and separates the several files that can define one
-- selector. Same `src` detection as import_spec; outside a src tree the bare
-- directory name is the best available label.
local function src_dir_label(file)
  local src = file:match("(.*/webapp/src)/") or file:match("(.*/src)/")
  local dir = vim.fs.dirname(file)
...
local function build_tag_items(root, idx)
  ...
  for sel, entries in pairs(idx) do
    ...
    for _, e in ipairs(entries) do
      items[#items + 1] = { selector = sel, file = e.file, dir = src_dir_label(e.file) }
    end
  end
...
function M.template_tag_completions(cb)
  local buf = vim.api.nvim_get_current_buf()
  if vim.bo[buf].filetype ~= "typescript" then return cb(nil) end
  if not in_template(buf) or in_attr_value(buf) then return cb(nil) end
  -- `<` then tag-name characters only: whitespace, `>` or an attribute in
  -- between means the name is already written and attribute completion owns the
  -- cursor. A `<` used as a comparison in a binding expression (`[x]="a <b"`)
  -- still matches -- harmless, since nothing there fuzzy-matches a selector.
  local before = vim.api.nvim_get_current_line():sub(1, vim.api.nvim_win_get_cursor(0)[2])
  local lt = before:find("<[%w%-]*$")
  if not lt then return cb(nil) end
  local meta = { lt_col = lt - 1, prefix = before:sub(lt + 1) }
  ...
  if meta.prefix == "" then return cb({}, meta) end

  local root = search_root(vim.api.nvim_buf_get_name(buf))
  selector_index.get(root, rg_run, function(idx)
...
local function decorated_class(dec)
  local p = dec:parent()
  if not p then return nil end
  if p:type() == "class_declaration" then return p end
  return child_by_type(p, "class_declaration")
end
...
local function decorator_pair(obj, key, src)
  for pair in obj:iter_children() do
    if pair:type() == "pair" then
...
local function decorator_option(obj, key, src)
  local pair = decorator_pair(obj, key, src)
  local v = pair and pair:field("value")[1]
  return v and vim.treesitter.get_node_text(v, src)
end

local function parse_component(file)
  local src = table.concat(vim.fn.readfile(file), "\n")
  local ok, parser = pcall(vim.treesitter.get_string_parser, src, "typescript")
  if not ok or not parser then return nil end
  local tree = (parser:parse() or {})[1]
  ...
  each_node(tree:root(), "decorator", function(dec)
    ...
    if obj and obj:type() == "object" then
      ...
      local raw = decorator_option(obj, "standalone", src)
      if raw then standalone = raw == "true" end
      local sel = decorator_option(obj, "selector", src)
      selector = sel and (sel:gsub("['\"]", ""))
    end
    info = {
      class = class,
      standalone = standalone,
      spec = import_spec(file, class),
      selector = selector,
      inputs = #inputs_for_file(file),
    }
  end)
  return info
end

local info_cache = {} -- file -> { mtime = <sec>, info = <table|false> }

-- Component metadata for `file` -- { class, standalone, spec, selector, inputs }
-- -- the documentation body behind a tag completion item. Cached per file+mtime
-- like inputs_for_file, misses included (a non-component .ts is a stable nil).
function M.component_info(file)
  local st = vim.uv.fs_stat(file)
  if not st then return nil end
  local c = info_cache[file]
  if c and c.mtime == st.mtime.sec then return c.info or nil end
  local info = parse_component(file)
  info_cache[file] = { mtime = st.mtime.sec, info = info or false }
  return info
end

-- ── auto-import wiring for an accepted tag completion ──────────────────────
-- Accepting `<fl-button` only compiles if the component is also reachable from
-- the consumer: imported at the top of the file AND listed in the consumer's own
-- `@Component({ imports: [...] })`. Both are plain text edits handed back as
-- `additionalTextEdits`, so blink applies them with the tag in one undo block.
-- NgModule-declared components (`standalone: false` -- 3837 of them in the GAF
-- webapp against 1438 standalone) instead need an `imports` edit in a *different*
-- file, so they are out of scope here: we wire nothing and say why, and
-- build_ngmodule_plan below plans that other file's edit.

-- LSP-shaped range for a node, so a caller can reason about the decorator's
-- position without holding a treesitter node (which dies on the next parse).
...
local function consumer_decorator(bufnr)
  ...
  local dname, call = decorator_call(dec, bufnr)
  if dname ~= "Component" or not call then return nil end
  local args = call:field("arguments")[1]
  local obj = args and args:named_child(0)
  if not obj or obj:type() ~= "object" then return nil end

  local cls = decorated_class(dec)
  local nn = cls and cls:field("name")[1]
  -- Same tri-state as component_info: absent means the decorator says nothing.
  local standalone
  local raw = decorator_option(obj, "standalone", bufnr)
  if raw then standalone = raw == "true" end
  local ipair = decorator_pair(obj, "imports", bufnr)
...
function M.consumer_info(bufnr)
  ...
  return {
    ...
    decorator = node_range(d.obj),
    imports_range = d.imports and node_range(d.imports) or nil,
  }
end
...
-- the middle of a hand-ordered list (`[CommonModule, DirectivesModule, …,
-- NasaReclamationLandingSharedModule, ComponentsModule]`) reads as noise in the
-- diff. Formatting follows the array we found -- one line stays one line, a line
-- per entry gets a line, trailing comma matched -- so prettier has nothing to say.
local function imports_entry_edit(bufnr, arr, name)
  ...
  if #entries == 0 then -- `imports: []`
    local open = child_by_type(arr, "[")
    if not open then return nil end
    local _, _, er, ec = open:range()
    return insert_at(er, ec, name)
  end
  ...
  if before then
    ...
    return insert_at(sr, sc, multiline and (name .. ",\n" .. string.rep(" ", sc)) or (name .. ", "))
  end

  local last = entries[#entries]
  local _, lsc, ler, lec = last:range()
  if not multiline then
    return insert_at(ler, lec, ", " .. name)
  end
  ...
  return insert_at(ler, comma and lec + 1 or lec,
...
local function imports_key_edit(anchor, name)
  ...
  return insert_at(sr, sc, "imports: [" .. name .. "],\n" .. string.rep(" ", sc))
end
...
-- An LSP TextEdit adding `name` to the enclosing @Component's `imports` array, or
-- nil when the consumer is NgModule-declared, `name` is already there, or there
-- is no component at the cursor. The edit lives in the decorator, always below
-- the import statements build_import_edit touches, so the two never overlap and
-- can travel together in one `additionalTextEdits`.
function M.build_imports_array_edit(bufnr, name)
  local d = consumer_decorator(bufnr)
  if not d then return nil end
  if d.standalone == false then return nil end -- NgModule consumer: build_ngmodule_plan
  return decorator_imports_edit(bufnr, d, name)
end

-- What the blink source calls when a tag completion is accepted: given the file
-- the completed selector resolves to, the edits that make the tag compile.
-- Returns `edits, info` -- info is component_info(target_file) plus a `reason`
-- when there are no edits, so the source can explain the no-op instead of
-- silently inserting a tag that won't render.
function M.build_component_edits(bufnr, target_file)
  local info = M.component_info(target_file)
  if not info then return {}, { reason = "not an Angular component" } end
  info = vim.tbl_extend("force", {}, info) -- component_info hands back its cache
  ...
  if consumer.standalone == false then
    info.reason = "consumer is NgModule-declared"
    return {}, info
  end
  ...
  local array_edit = M.build_imports_array_edit(bufnr, info.class)
...
-- it, in a different file. Resolving that is a data problem
-- (lua/gaf/angular/module_index.lua), and what comes out here is a PLAN: which file,
-- which symbol, which edits. Nothing is applied, opened or prompted; the caller
-- owns that, because editing a file the user isn't looking at is a decision only
-- they can make.

-- Import specifier for a module record, memoized on the record. import_spec walks
-- ancestor barrels reading `index.ts` files -- far too much to spend on all 895
-- exporting modules at build time when one lookup touches a handful.
local function module_spec(m)
  if m.spec == nil then m.spec = import_spec(m.file, m.module_class) or false end
...
local function ngmodule_decorator(bufnr, class)
  ...
  each_node(tree:root(), "decorator", function(dec)
    ...
    local args = call:field("arguments")[1]
    local obj = args and args:named_child(0)
    if not obj or obj:type() ~= "object" then return end
    local ipair = decorator_pair(obj, "imports", bufnr)
    local ival = ipair and ipair:field("value")[1]
    local first
...
local function module_root(bufnr)
  return search_root(vim.api.nvim_buf_get_name(bufnr))
end

-- Every module that directly exports `class`, as { module_class, file, spec }.
function M.modules_exporting(class, cb)
  module_index.get(module_root(0), rg_run, function(idx)
    local out = {}
    for _, m in ipairs(idx.exporters[class] or {}) do
      out[#out + 1] = module_entry(m)
    end
    cb(out)
  end)
end

-- The NgModule that declares `class`, as { module_class, file }, or nil.
function M.module_for_component(class, cb)
  module_index.get(module_root(0), rg_run, function(idx)
    local m = idx.declarer[class]
    cb(m and { module_class = m.module_class, file = m.file } or nil)
  end)
end
...
function M.build_ngmodule_plan(bufnr, target_file, cb)
  ...
  module_index.get(root, rg_run, function(idx)
...
function M.reindex()
  ...
  selector_index.get(root, rg_run, function(idx)
    ...
    notify(n .. " selectors indexed under " .. root)
  end)
end
