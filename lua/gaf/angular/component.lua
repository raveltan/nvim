...
-- (verified in the GAF webapp). Prefers the nearest ancestor barrel re-exporting
-- `name`, so this yields `@freelancer/ui/button` over the deep
-- `@freelancer/ui/button/button-size`; falls back to the src-relative path.
-- Memoized: the walk reads an `index.ts` per ancestor directory, and the NgModule
-- planner asks the same question of every module that exports a class.
...
-- field. `pending` holds decorators that preceded the member as siblings, which
-- is how the TS parser attaches one to a setter method (on a field it's a child).
