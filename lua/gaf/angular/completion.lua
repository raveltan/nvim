...
-- when the cursor isn't in tag-name position. `lt_col` is the 0-indexed column of
-- the `<`, so the source replaces the partial tag rather than appending to it.
function M.tag_completions(cb)
  ...
  -- A bare `<` is also every `<div` and `<ng-container`, and a 4800-item menu is
  -- no use at that width. Answer "tag position, nothing yet" without touching the
  -- index -- otherwise the first `<` in any template pays for the repo-wide rg on
